...
# Add the SwinTransformer/models directory to sys.path
current_dir = os.path.dirname(os.path.abspath(__file__))
swin_path = os.path.abspath(os.path.join(current_dir, "../SwinTransformer/models"))
if swin_path not in sys.path:
    sys.path.append(swin_path)
...
class RoPEWindowAttention(WindowAttention):
    ...
    def forward(self, x, mask=None):
        ...
        if self.rope_mixed:
            # Mixed RoPE (learnable freqs): compute cis on-the-fly.
            if (not self._viyarn_need_scaling) or (self.viyarn_alpha <= 0.0):
                freqs_cis = compute_cis(self.rope_freqs, self.rope_t_x, self.rope_t_y)
            elif self.viyarn_alpha >= 1.0:
